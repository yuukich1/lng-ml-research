...
Проект теперь использует `json/jsonl` как единственный входной raw-формат.
Старые `training_dataset.csv` больше не используются как источник для модели и STS-анализа.
